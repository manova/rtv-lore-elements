...
- [ ] One hardcoded `Note_HelloWorld.tres` registered into `LT_Master`.
- [ ] Pick up the note in-game, see it in inventory.
- [ ] Right-click → "Read" → modal dialog showing fixed text.
- [ ] Confirm `_lib.skip_super()` semantics if hooking a replace surface.
...
- [ ] MCM config: spawn rate multiplier, journal hotkey customization.
