...
In-game testing showed the action can disappear after registration, most likely when the game's input preferences rebuild the active `InputMap`. The journal still uses the registry as the primary path, but its event handler repairs a missing `rtv_lore_open_journal` action and also recognizes a direct, non-echo `KEY_J` key event. This keeps the default hotkey usable without widening the public API or adding a preferences patch.
