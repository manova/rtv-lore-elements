...
The journal uses a `CanvasLayer` and attaches to the visible Interface node when inventory is open, otherwise the current scene. Opening the journal or reader saves the current mouse mode and `gameData.freeze`, then confines the mouse and freezes gameplay so the modal can be clicked from pure gameplay as well as inventory. The reader sits above the journal when opened from a journal entry; closing that reader returns to the journal list without releasing controls between overlays. While the journal is open, `uimanager-_input` is skipped so vanilla interface/settings hotkeys cannot affect the UI behind it; Escape/settings and `J` close only the topmost lore overlay first. Journal close does not reset the inventory interface, while reader close keeps the existing cleanup discipline for context-menu read actions.
