...
In-game testing showed the action can disappear after registration, most likely when the game's input preferences rebuild the active `InputMap`. The journal still uses the registry as the primary path, but its event handler repairs a missing `rtv_lore_open_journal` action. Only the repair path recognizes the direct default `KEY_J` event, so a future user rebind is not bypassed when the action already exists.

## Layering and focus

The journal uses a `CanvasLayer` above the Phase 2 reader layer and attaches to the visible Interface node when inventory is open, otherwise the current scene. Opening the journal or reader saves the current mouse mode and `gameData.freeze`, then confines the mouse and freezes gameplay so the modal can be clicked from pure gameplay as well as inventory. Closing restores the prior values; opening a note from the journal transfers modal ownership directly to the reader without releasing controls between overlays. While the journal is open, `uimanager-_input` is skipped so vanilla interface/settings hotkeys cannot affect the UI behind it; Escape/settings and `J` close only the journal. Journal close does not reset the inventory interface, while reader close keeps the existing cleanup discipline for context-menu read actions.
